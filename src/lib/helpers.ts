...
import { AnnotationIgnoreField, Tag } from '../types/interface'
...
export function mapTag(tagName: string, tagPrefix: string): Tag {
  ...
  return {
    ...
    permalink: `/${tagPrefix}/${makeSlugText(tagName)}/`
  }
}
...
export function mapColorClass(color?: string): string | null {
  switch (color) {
    ...
    case 'pink_background':
      return 'bg-pink-100 dark:bg-pink-700'
    case 'red_background':
...
export function getCreatedDate(block: Block, createdId: string) {
...
export function getModifedDate(block: Block, modifiedId: string) {
...
export const defaultMapImageUrl = (url: string, block: Block): string | null => {
  ...
  // more recent versions of notion don't proxy data image, unsplash images && cloudinary
  if (
    url.startsWith('data:') ||
