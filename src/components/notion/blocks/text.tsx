...
export default function BlockText(props: TextProps) {
  ...
  if (
    ...
    if (
      new URL(props.richText.href).hostname.includes(process.env.NEXT_PUBLIC_SITE_DOMAIN as string) &&
      !props.richText.href.includes('@')
    ) {
      const uri = getUriFromUrl(props.richText.href, process.env.NEXT_PUBLIC_SITE_DOMAIN as string)
      return (
        <Link
          className={generateTextAnnotationClasses(props.richText.annotations, props.ignore as AnnotationIgnoreField[])}
          href={uri}
        >
      ...
      return (
        <a
          className={cn(
            'notion-link',
            generateTextAnnotationClasses(props.richText.annotations, props.ignore as AnnotationIgnoreField[])
          )}
