...
export default function CVGroup({ cv }: { cv: CVGroupType }) {
  return (
    <div className="bg-white p-6 rounded-xl">
      <h3 className="font-semibold text-2xl leading-none text-neutral-900">{cv.name}</h3>
      <div className="flex flex-col divide-neutral-200 divide-y mt-6">
        {cv.lists.map(item => (
          <div
            key={item.id}
          ...
          >
            ...
            <div className="w-full sm:w-3/4 text-center sm:text-left">
              <h4 className="text-lg font-medium text-neutral-900 sm:font-semibold sm:text-base">
                {item.job}
              </h4>
              {item.activity?.map((activity, index) => (
                <p key={index} className="block text-neutral-500 text-sm mt-3.5">
                  {activity}
                </p>
              ))}
            </div>
