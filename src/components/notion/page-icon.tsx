...
export const PageIconImpl: React.FC<{
  ...
  defaultIcon?: string
}> = ({ block, className, wrapperClassName, inline = true, hideDefaultIcon = false, defaultIcon }) => {
  ...
  if (isIconBlock(block)) {
    ...
    if (icon && isUrl(icon)) {
      ...
      isImage = false
      content = (
        <span className={cn([className], 'notion-page-icon icon-emoji')} role='img' aria-label={icon}>
          {icon}
        </span>
      )
    }
