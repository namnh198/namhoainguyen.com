...
import { getBlockIcon, getBlockParentPage, getPageTableOfContents, getTextContent, uuidToId } from 'notion-utils'
import * as React from 'react'
import { cn, getListNumber, isUrl } from '@/lib/utils'
...
import { GoogleDrive } from './blocks/google-drive'
import BlockToggle from './blocks/toggle'
import BlockHeadingToggle from './blocks/toggle-heading'
...
export const Block: React.FC<BlockProps> = (props) => {
  ...
  switch (block.type) {
    ...
    case 'page':
      if (level === 0) {
        const { page_cover_position = defaultPageCoverPosition, page_full_width, page_small_text } = block.format || {}
                ...
                <div className='notion-page-scroller'>
                  <main
                    className={cn(
                      'notion-page notion-prose dark:notion-prose-invert',
                      {
                        ...
                        'notion-full-width': !!page_full_width,
                        'notion-small-text': !!page_small_text
                      },
                      'notion-full-page',
                      [bodyClassName]
                    )}
                  >
                    {block.type !== 'collection_view_page' && (
                      <div
                        className={cn('notion-page-content', {
                          'notion-page-content-has-aside': hasAside,
                          'notion-page-content-has-toc': hasToc
                        })}
                      >
    ...
    case 'sub_sub_header': {
      ...
      const classNameStr = cn(
        {
          'notion-h notion-h1': isH1,
          'notion-h notion-h2': isH2,
          'notion-h notion-h3': isH3
        },
        [`notion-${blockColor}`, indentLevelClass, blockId]
      )
      ...
      const innerHeader = (
        <>
          ...
          {!block.format?.toggleable && (
            <a className={cn('opacity-0 group-hover:opacity-100')} href={`#${anchor}`} title={title}>
              <CiLink />
            </a>
          )}
        </>
      ...
      if (isH1) {
        ...
        headerBlock = (
          <h3 id={anchor} className={cn(classNameStr, headingCommonClasss, 'my-0 group')} data-id={id}>
            {innerHeader}
          </h3>
        )
      }
    ...
    case 'divider':
      return <hr className={cn('notion-hr', [blockId])} />
    ...
    case 'text': {
      ...
      const blockColor = block.format?.block_color
      const notionBlockColor = `notion-${blockColor}`

      return (
        <div
          className={cn('notion-text', [basicBlockGap], [blockId], {
            [notionBlockColor]: notionBlockColor
          })}
        >
    ...
    case 'numbered_list': {
      const wrapList = (content: React.ReactNode, start?: number) =>
        block.type === 'bulleted_list' ? (
          <ul className={cn('notion-list', 'notion-list-disc', [blockId])}>{content}</ul>
        ) : (
          <ol start={start} className={cn('notion-list', 'notion-list-numbered', [blockId])}>
            {content}
          </ol>
        )
    ...
    case 'column_list': {
      return <div className={cn('md:flex md:flex-nowrap md:gap-4 md:-my-2', blockId)}>{children}</div>
    }
    ...
    case 'table_of_contents': {
      ...
      return (
        <div className={cn('notion-table-of-contents', [`notion-${blockColor}`, blockId])}>
          {toc.map((tocItem) => (
    ...
    case 'to_do': {
      ...
      return (
        <div className={cn('notion-to-do', [blockId])}>
          <div className='flex items-baseline gap-2 my-2'>
            <div className='w-4 h-4'>
              {isChecked && <BsCheckSquareFill className='text-slate-500 mt-0.5' />}
    ...
    case 'table':
      return (
        <div className={cn([basicBlockGap], 'overflow-auto notion-scrollbar')}>
          <table className={cn('notion-simple-table table-auto my-0', [blockId])}>
            <tbody
              className={cn({
    ...
    case 'table_row': {
      ...
      return (
        <tr className={cn('notion-simple-table-row', [`notion-${backgroundColor}`, blockId])}>
          {order.map((column) => {
