            ...
            >
              ...
              >
                ...
                {data && !data?.[0]?.isFake && query && (
                  <>
                    {data.length === 0 && (
                      <div className='p-4 text-center text-base'>{props.noResultsMessage || 'No results found.'}</div>
                    )}
                    {data.length > 0 && (
                      <div className={cn('flex flex-col divide-y overflow-hidden')}>
                        <div
                          ref={containerRef}
                          className={cn('flex flex-col divide-y divide-slate-150 overflow-auto notion-scrollbar')}
                        >
                          {data.map((item, index) => {
                            ...
                            return (
                              ...
                              >
                                <div className='mt-0.5'>
                                  {!item.isBookPost && <IoDocumentTextOutline className='text-xl opacity-80' />}
                                  {item.isBookPost && <IoBookOutline className='text-xl opacity-80' />}
                                </div>
                                <div className={cn('w-full flex flex-col text-slate-900 gap-1')}>
                                  <div className={cn('w-full text-base flex items-center justify-between')}>
                                    <div
                                      className={cn(
                                        {
