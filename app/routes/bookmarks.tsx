          ...
          {bookmarks.map((bookmark) => (
            <div key={bookmark.title}>
              <div className="flex items-center gap-3 mb-4 pb-3 border-b">
                ...
                <span className="text-xs text-text-3 font-bold uppercase">
                  {bookmark.title}
                </span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3.5">
                {bookmark.list.map((bmItem) => (
                  ...
                  >
                    <span className="absolute inset-0 top-0 left-0 bottom-0 transition-opacity opacity-0 group-hover/bookmark:opacity-100 bg-linear-[180deg,var(--accent),var(--accent-2)] w-0.75 rounded-tl-lg rounded-bl-lg"></span>
                    <span className="flex items-start justify-between gap-2">
                      <span className="text-[0.9rem] leading-[1.35] font-semibold text-text">
                        {bmItem.name}
                      </span>
                      <IconExternalLink
                        stroke={2}
                    ...
                    {bmItem.excerpt && (
                      <span className="text-[0.8rem] leading-normal text-text-2">
                        {bmItem.excerpt}
                      </span>
                    )}
