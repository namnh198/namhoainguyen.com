    ...
    <div className='relative shadow-md bg-white dark:bg-neutral-900 border border-neutral-200/70 dark:border-slate-600 group transition duration-200 ease-in-out hover:-translate-y-0.5 hover:-translate-x-0.5 p-5 rounded-3xl'>
      ...
      <div className='flex flex-col'>
        <div className='flex flex-wrap gap-1.5'>
          <Button variant={getColorBadge(project.type)} size='sm' className='rounded-full font-medium'>
            {project.type || 'Projects'}
          </Button>
        </div>

        <h2 className='block mt-4  font-semibold text-base text-neutral-900 dark:text-neutral-100'>{project.title}</h2>
        {project.description && (
          <p className='block  text-neutral-500 dark:text-neutral-400 text-sm mt-3.5 mb-4'>{project.description}</p>
        )}
        {project.techs && (
          <div className='relative flex flex-wrap gap-1.5'>
            {project.techs.map((tech, index) => (
              <Tech key={index} techId={tech} />
            ))}
          </div>
        )}
      </div>
