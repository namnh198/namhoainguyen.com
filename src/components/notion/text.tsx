...
import { NotionComponents } from '@/types/types'
...
export const Text: React.FC<{
  ...
  const { components: ctxComponents, recordMap, mapPageUrl, mapImageUrl, rootDomain, blockOptions } = useNotionContext()
        ...
        const formatted = decorations.reduce((element: React.ReactNode, decorator) => {
          switch (decorator[0]) {
            case 'p': {
              ...
              const postSlug = linkedBlock?.properties?.[`${process.env.NEXT_PUBLIC_ID_NOTE_SLUG_KEY}`]?.[0]?.[0]
            ...
            case '‣': {
              ...
              switch (linkType) {
                case 'u': {
                  ...
                  return (
                    <GracefulImage className='notion-user' src={mapImageUrl(user.profile_photo, block)} alt={name} />
                  )
                }
            ...
            case 'a': {
              ...
              if ((v[0] === '/' || v.includes(rootDomain!)) && id) {
                ...
                const href = linkedBlock
                  ? mapNoteUri(linkedBlock?.properties?.[`${process.env.NEXT_PUBLIC_ID_NOTE_SLUG_KEY}`]?.[0]?.[0])
                  : v.includes(rootDomain!)
                  ? v
                ...
                if (hostName === 'localhost' || hostName?.includes(blockOptions?.siteDomain || 'cannotBeIncluded')) {
                  return (
