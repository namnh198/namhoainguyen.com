...
export default function BlockCode(props: BlockCodeProps) {
  ...
  const syntaxWraper = (
    <SyntaxHighlighter
      ...
      style={prism}
      className={cn(
        '!my-0 syntax-highlighter-pre notion-scrollbar notion-scrollbar-small border !bg-slate-50',
        'max-h-[300px]'
      )}
    ...
    <div className={cn(className, 'group flex flex-col gap-2')}>
      {block.format?.code_preview_format !== 'preview' && (
        <div className={`language-${formatCodeLang(language)} syntax-highlighter relative text-[14px]`}>
          {syntaxWraper}
          <div
            className={cn('!absolute right-2 top-2 duration-100 hover:cursor-pointer group-hover:opacity-100', {
              'opacity-0': !copied
            })}
