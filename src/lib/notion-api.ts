...
import { BookmarkPreview, CollectionInstanceNotion, NotionSorts } from '@/types/interface'
...
export async function queryDatabaseImpl(opts: {
  ...
  try {
    ...
    if (data && data['has_more'] && data['next_cursor'] && pageSize && pageSize >= notionMaxRequest) {
      while (data!['has_more']) {
 ...
 */
export const retrieveBlockChildren = async (pageId: string, pageSize?: number, startCursor?: string) => {
...
export async function getBlocks(
  ...
  for (const block of blocks) {
    ...
    if (get(block, `${block.type}.rich_text`) && !!getPageUri) {
      const parsedMention = await parseMention(get(block, `${block.type}.rich_text`) as any, getPageUri)
